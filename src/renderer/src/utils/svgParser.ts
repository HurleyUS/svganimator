...
import { SVGElementNode } from '../store/useStore'
...
export function parseSVG(svgString: string): SVGElementNode[] {
  ...
  const viewBoxParts = viewBox?.split(/[\s,]+/).map(Number).filter((value) => Number.isFinite(value))
  const sourceBounds = viewBoxParts?.length === 4
  ...
  const parseNode = (node: Element): SVGElementNode | null => {
    const allowedTypes = ['path', 'rect', 'circle', 'ellipse', 'line', 'polyline', 'polygon', 'g', 'text', 'image', 'use', 'defs', 'clipPath', 'mask', 'linearGradient', 'radialGradient', 'stop', 'filter', 'feGaussianBlur', 'feOffset', 'feMerge', 'feMergeNode', 'feColorMatrix', 'feComposite', 'feFlood', 'feMorphology', 'feBlend', 'feTurbulence', 'feDisplacementMap', 'feSpecularLighting', 'feDiffuseLighting', 'fePointLight', 'feDistantLight', 'feSpotLight']
  ...
  for (let i = 0; i < svgRoot.children.length; i++) {
    ...
    if (!child) continue
    if (child.type === 'defs' || child.type === 'clipPath' || child.type === 'mask' || child.type.includes('gradient') || child.type === 'filter') {
      defs.push(child)
    } else {
      content.push(child)
    }
