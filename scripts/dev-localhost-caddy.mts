...
type CommandRunner = (command: string, args: Array<string>) => SpawnSyncReturns<string>;
...
export function writeProjectSnippet(snippetPath: string, host: string, port: number) {
  const content = `${host} {\n\treverse_proxy 127.0.0.1:${port}\n}\n`;
  writeFileSync(snippetPath, content, "utf8");
}

function printCommandResult(result: SpawnSyncReturns<string>, output = process) {
  if (result.stdout) {
...
function caddyAdminUnavailable(result: SpawnSyncReturns<string>) {
  ...
  return (
    loadUrls.some((url) => output.includes(url)) && output.includes("connect: connection refused")
  );
}
